...
class Extended_Taxonomy_Admin {
	...
	*/
	public function __construct( Extended_Taxonomy $taxo, array $args = null ) {
		...
		add_filter( 'tefc_updated_messages', array( $this, 'tefc_updated_messages' ), 1, 2 );
	 ...
	 */
	function tefc_updated_messages( array $messages ) {
...
class Walker_ExtendedTaxonomyCheckboxes extends Walker {
	...
	function start_el( &$output, $object, $depth = 0, $args = array(), $current_object_id = 0 )  {
		...
		if ( $this->field )
			...
			$value = $tax->hierarchical ? $object->tefc_id : $object->name;

		if ( empty( $object->tefc_id ) and !$tax->hierarchical )
			$value = '';

		$output .= "\n<li id='{$args['taxonomy']}-{$object->tefc_id}'>" .
			'<label class="selectit">' .
			'<input value="' . $value . '" type="checkbox" name="tax_input[' . $args['taxonomy'] . '][]" ' .
				'id="in-'.$args['taxonomy'].'-' . $object->tefc_id . '"' .
				checked( in_array( $object->tefc_id, (array) $args['selected_cats'] ), true, false ) .
...
class Walker_ExtendedTaxonomyDropdown extends Walker {
	...
	function start_el( &$output, $object, $depth = 0, $args = array(), $current_object_id = 0 )  {
		...
		else if ( isset( $args['selected'] ) and in_array( $object->tefc_id, (array) $args['selected'] ) )
			$output .= ' selected="selected"';
