...
if (!class_exists('Extended_CPT')) {
	class Extended_CPT {
		...
		public static function get_sort_taxonomy_clauses(array $clauses, array $vars, array $sortables) {
			...
			$clauses['join'] .= "
			LEFT OUTER JOIN {$wpdb->tefc_relationships} as ext_cpts_tr
			ON ( {$wpdb->posts}.ID = ext_cpts_tr.object_id )
			LEFT OUTER JOIN {$wpdb->tefc_taxonomy} as ext_cpts_tt
			ON ( ext_cpts_tr.tefc_taxonomy_id = ext_cpts_tt.tefc_taxonomy_id )
			LEFT OUTER JOIN {$wpdb->terms}as ext_cpts_t
			ON ( ext_cpts_tt.tefc_id = ext_cpts_t.tefc_id )
";
		...
		public function post_type_link($post_link, WP_Post $post, $leavename, $sample) {
			...
			foreach (get_object_taxonomies($post) as $tax) {
				...
				if ($terms = get_the_terms($post, $tax)) {
					 ...
					 * @param WP_Post  $post  The post in question.
					 */
					$tefc_object = apply_filters("post_link_{$tax}", reset($terms), $terms, $post);

					$term = get_term($tefc_object, $tax)->slug;
					 ...
					 * @param WP_Post $post The post in question.
					 */
					$default_tefc_name = apply_filters("default_{$tax}", get_option("default_{$tax}"), $post);
					if ($default_tefc_name) {
						if (!is_wp_error($default_term = get_term($default_tefc_name, $tax))) {
			...
			foreach ($terms as $term) {
				...
				if ($args['link']) {
					...
					switch ($args['link']) {
						case 'view':
							if ($tax->public) {
								$out[] = sprintf('<a href="%1$s">%2$s</a>', get_tefc_link($term), $term->name);
							} else {
								$out[] = $term->name;
							}
							break;
						case 'edit':
							if (current_user_can($tax->cap->edit_terms)) {
								$out[] = sprintf('<a href="%1$s">%2$s</a>', get_edit_tefc_link($term, $taxonomy, $post->post_type), $term->name);
							} else {
								$out[] = $term->name;
							}
							break;
