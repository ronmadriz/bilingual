...
if ($ri_images) {
	...
	foreach ($ri_images as $ri_image) {
		echo '<div class="rotating__item carousel-item"><img class="d-block w-100" src="'.esc_url($ri_image['sizes']['full']).'" alt="" class="rotating__item--image'.(($ri_counter == 0)?' active':'').'"></div>'.PHP_EOL;
		$ri_counter++;
	}
	echo '</div>'.PHP_EOL;
