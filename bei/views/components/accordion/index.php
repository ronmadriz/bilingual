...
if (have_rows('acc_accordion_item')) {
	...
	while (have_rows('acc_accordion_item')) {
		...
		echo '</div>'.PHP_EOL;
		echo '<div id="'.$acc_item_id.'_content-'.$acc_counter.'" class="collapse '.(($acc_counter == 0?'show':'')).'  accordion__content" aria-labelledby="'.$acc_item_id.'" data-parent="#'.$acc_id.'-accordion">'.PHP_EOL;
