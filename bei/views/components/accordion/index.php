...
if (have_rows('acc_accordion_item')) {
	...
	while (have_rows('acc_accordion_item')) {
		...
		echo '<h3 class="card-header accordion__header" id="'.$acc_id.'-'.$acc_counter.'"><a class="accordion__header--btn" data-toggle="collapse" data-target="#'.$acc_id.$acc_counter.'-content" aria-expanded="'.(($acc_counter == 0?'true':'false')).'" aria-controls="'.$acc_id.$acc_counter.'-content"><i class="accordion__icon"></i> '.$acc_item_title.'</a></h3>'.PHP_EOL;
