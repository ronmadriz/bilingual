...
if (have_rows('acc_accordion_item')) {
	...
	while (have_rows('acc_accordion_item')) {
		...
		echo '<div class="card-header accordion__header" id="'.$acc_item_id.'">'.PHP_EOL;
		echo '<button class="btn btn-link" data-toggle="collapse" data-target="#'.$acc_item_id.'_content" aria-expanded="true" aria-controls="'.$acc_item_id.'_content">'.$acc_item_title.'</button>'.PHP_EOL;
		echo '</div>'.PHP_EOL;
