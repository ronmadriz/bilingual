...
if (have_rows('acc_accordion_item')) {
	...
	while (have_rows('acc_accordion_item')) {
		...
		echo '<h3 class="accordion__header--text"><a class="accordion__header--btn" data-toggle="collapse" data-target="#'.$acc_id.$acc_counter.'-content" aria-expanded="'.(($acc_counter == 0?'true':'false')).'" aria-controls="'.$acc_id.$acc_counter.'-content"><i class="accordion__icon"></i> '.$acc_item_title.'</a></h3>'.PHP_EOL;
		echo '</div>'.PHP_EOL;
