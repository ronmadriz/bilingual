...
if (have_rows('acc_accordion_item')) {
	...
	while (have_rows('acc_accordion_item')) {
		...
		echo '<button class="accordion__header--btn" data-toggle="collapse" data-target="#'.$acc_id.$acc_counter.'-content" aria-expanded="'.(($acc_counter == 0?'true':'false')).'" aria-controls="'.$acc_id.$acc_counter.'-content"><i class="accordion__icon"></i> '.$acc_item_title.'</button>'.PHP_EOL;
		echo '</div>'.PHP_EOL;
