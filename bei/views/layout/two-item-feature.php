...
if (have_rows('first_item')) {
	while (have_rows('first_item')) {
		...
		echo '<div class="twoItemFeature__item col-12 col-6">'.PHP_EOL;
		echo (!empty($tif_1__icon)?'<span class="twoItemFeature__icon">'.file_get_contents(get_stylesheet_directory_uri().'/sprites/'.$tif_1__icon.'.svg').'</span>'.PHP_EOL:'');
...
if (have_rows('second_item')) {
	while (have_rows('second_item')) {
		...
		echo '<div class="twoItemFeature__item col-12 col-6">'.PHP_EOL;
		echo (!empty($tif_2__icon)?'<span class="twoItemFeature__icon">'.file_get_contents(get_stylesheet_directory_uri().'/sprites/'.$tif_2__icon.'.svg').'</span>'.PHP_EOL:'');
