...
if ($childprograms->have_posts()) {
	...
	while ($childprograms->have_posts()) {
		...
		$childprogram__img     = get_the_post_thumbnail_url($size = 'medium');
		...
		echo '<a href="'.$childprogram__link.'" class="tiled__image--link"><img src="'.esc_url($childprogram__img).'" alt="'.$childprogram__title['alt'].'" class="tiled__image card-img-top img-fluid"></a>'.PHP_EOL;
		...
		echo (!empty($childprogram__link)?'<a href="'.$childprogram__link['url'].'" class="tiled__link">'.$childprogram__link['title'].' <span class="tiled__link--icon">'.file_get_contents(get_stylesheet_directory_uri().'/sprites/arrow.svg').'</span></a>'.PHP_EOL:'');
